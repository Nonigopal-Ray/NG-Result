...
class ColorPDF(FPDF):
    ...
    def footer(self):
        ...
        # Adding clickable URL in PDF Footer
        ...
        self.set_text_color(100, 100, 100)
        self.set_font("Helvetica", "I", 8)
        self.cell(93, 5, f" | Dinajpur Board SSC {self.year} Result System", align="L")
    ...
    def draw_colored_grades(self, x, y, width, height, text):
        ...
        for item in items:
            ...
            # Line Wrap logic to prevent text overflowing the column
            if current_x + item_width > x + width - 1.5:
                current_x = x + 1.5
                current_y += line_height
                if current_y + line_height > y + height:
                    break  # Stop rendering if exceeding row boundary
            ...
            self.text(current_x, current_y + 2.5, item_str)
            current_x += item_width

        self.set_text_color(0, 0, 0)


def calculate_required_height(pdf, width, text, line_h=3.8):
    """Accurately calculates total row height required for wrapping text."""
    items = [i.strip() for i in text.split(',') if i.strip()]
    ...
    for item in items:
        ...
        if current_x + item_w > width - 1.5:
            lines += 1
            current_x = 1.5 + item_w
        else:
            current_x += item_w
            
    return max(7, (lines * line_h) + 2.5)
...
def generate_pdf(dataframe, institute_info, year):
    ...
    for _, row in dataframe.iterrows():
        ...
        # Dynamically determine row height so all subject grades fit perfectly
