...
st.markdown('<div class="developer-badge">✨ App Developed by: <a style="color: white; text-decoration: underline;" href="https://www.facebook.com/nonigopalrays" target="_blank">Nonigopal Ray</a> ✨</div>', unsafe_allow_html=True)
st.write("---")
...
class ColorPDF(FPDF):
    ...
    def footer(self):
        ...
        # Clickable Facebook Link in Footer
        fb_url = "https://www.facebook.com/nonigopalrays"
        self.cell(150, 5, f"App Developed by: Nonigopal Ray (Click to Visit FB Profile)", align="L", link=fb_url)
        
        self.set_text_color(100, 100, 100)
        self.cell(0, 5, f"Page {self.page_no()}/{{nb}}", align="R")

    def format_subject_lines(self, text, max_width):
        """Splits subject grade text into multiple lines with reduced font size."""
        self.set_font("Helvetica", "", 6)  # Reduced font size for Subject Grade
    ...
    def draw_colored_grades(self, x, y, line_lines):
        """Draws subject grades neatly with small font size."""
        self.set_font("Helvetica", "", 6)  # Reduced font size (6pt)
        curr_y = y + 2.2
        ...
        for line in line_lines:
            ...
            for item in line:
                parts = item.split(':')
                sub, grade = (parts[0], parts[1]) if len(parts) == 2 else (item, "")
                item_str = f"{sub}:{grade}, " if grade else f"{item}, "

                if grade == 'F':
                    self.set_text_color(211, 47, 47)  # Red color for Fail
                else:
...
def generate_pdf(dataframe, institute_info, year):
    ...
    pdf.cell(0, 5, f"BOARD OF INTERMEDIATE AND SECONDARY EDUCATION, DINAJPUR | SSC {year}", ln=True, align="C")
    ...
    for _, row in dataframe.iterrows():
        ...
        # Calculate dynamic row height based on subject grade lines
        grade_lines = pdf.format_subject_lines(sg, widths[5])
        num_lines = max(1, len(grade_lines))
        row_h = max(6.5, num_lines * 3.2 + 2.0)
...
# ---------------------------------------------------------
# Developer Photo Section (Fixed Facebook Embed Iframe)
# ---------------------------------------------------------
st.write("---")
st.subheader("👨‍💻 Developer Profile")

# Optimized HTML code to properly render Facebook Post Iframe with fallback button
fb_iframe_code = """
<!DOCTYPE html>
<html>
<head>
...
<body>
<div class="container">
    <iframe src="https://www.facebook.com/plugins/post.php?href=https%3A%2F%2Fweb.facebook.com%2Fnonigopalrays%2Fposts%2F122124760767127318%3A2714507345572582&show_text=true&width=500" 
            width="500" height="490" scrolling="no" frameborder="0" 
...
components.html(fb_iframe_code, height=510)

# Footer Developer Credit
st.markdown('<p style="text-align: center; color: #757575;"><b>Dinajpur Board SSC Result Processing System</b><br>Designed & Developed by <b><a href="https://www.facebook.com/nonigopalrays" target="_blank">Nonigopal Ray</a></b></p>', unsafe_allow_html=True)