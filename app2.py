...
def fetch_student_detail(
    ...
    """রিট্রাই লজিক সহ রেজাল্ট ফেচ করার উন্নত ফাংশন"""
    for attempt in range(max_retries):
        session = requests.Session()
        try:
            if str(year) == "2026":
                get_res = session.get(action_url, headers=session_headers, timeout=10)
                ...
                response = session.post(
                    action_url, data=payload, headers=session_headers, timeout=10
                )
            else:
...
def generate_pdf(dataframe, institute_info, year, is_eiin=True):
    ...
    # PDF Summary Header Table (Failed সেলের পরেই Absent যুক্ত করা হয়েছে)
    pdf.set_font("Helvetica", "B", 8)
    pdf.set_fill_color(240, 240, 240)
    
    # 6টি সেল সমানভাবে ভাগ করা হয়েছে (46*5 + 47 = 277mm)
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        if extracted_pairs:
            ...
            # পাসের হিসাব ও Absent ফিল্টারিং
            ...
            # EIIN অনুসন্ধানের ক্ষেত্রে পাসের হার ওয়েবসাইট থেকে যা আসবে সেটাই থাকবে।
            # শুধুমাত্র Individual রেজাল্টের ক্ষেত্রে ম্যানুয়ালি বের করা হার ব্যবহৃত হবে।
            if not is_eiin_search:
                institute_info["Percentage of Pass"] = str(calculated_pass_rate)
            elif institute_info.get("Percentage of Pass") in ["-", "0", "0.0", ""]:
                institute_info["Percentage of Pass"] = str(calculated_pass_rate)
...
if (
    ...
    # UI Metrics Card Display (Failed এর পর Absent যুক্ত)
