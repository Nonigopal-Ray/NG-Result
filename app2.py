...
def parse_institute_summary(raw_html, eiin_code):
    ...
    for i, el in enumerate(all_elements):
        ...
        if i + 1 < len(all_elements):
            ...
            ) and info["Institute"] in ["Unknown School", "Unknown Institute"]:
                if (
                    next_val
                    and len(next_val) > 3
    ...
    for key, pat_list in patterns.items():
        if info[key] in ["Unknown School", "Unknown Institute", "-"]:
            for pat in pat_list:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, td in enumerate(all_tds):
                ...
                if "name" in txt and "school" not in txt and "institute" not in txt and i + 1 < len(all_tds) and name == "Not Found":
                    possible_name = all_tds[i + 1].text.strip()
                    if len(possible_name) > 2 and not possible_name.isdigit():
                        name = possible_name

                if ("school" in txt or "institute" in txt) and i + 1 < len(all_tds) and school == "Not Found":
                    possible_school = all_tds[i + 1].text.strip()
                    if len(possible_school) > 2:
...
def generate_pdf(dataframe, institute_info, year):
    ...
    # PDF Headline: স্কুলের নাম বড় অক্ষরে দৃশ্যমান থাকবে
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        if extracted_pairs:
            ...
            # ব্যাকআপ স্কুলের নাম সেট করা (যদি ইনস্টিটিউট পার্সারে মিস হয়ে থাকে)
            if institute_info.get("Institute") in ["Unknown School", "Unknown Institute", ""]:
                for item in results_data:
                    if item.get("School") not in ["Not Found", "-", None, ""]:
                        institute_info["Institute"] = item.get("School")
                        break
...
if (
    ...
    school_name_display = info.get("Institute", "School Result Sheet")

    # রেজাল্ট শিটের টাইটেলে স্কুলের নাম প্রদর্শিত হবে
    if is_eiin_search:
        st.subheader(
            f"🏫 {school_name_display} (EIIN: {eiin_code})"
        )
        st.caption(
    ...
    # PDF ফাইলের নাম তৈরি (স্কুলের নাম সহ)
