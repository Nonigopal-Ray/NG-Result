...
def clean_school_name(name):
    """স্কুলের নাম থেকে EIIN কোড এবং পেছনের জেলা/থানার নাম বাদ দেওয়ার ফাংশন"""
    if not name:
        return ""
    # সামনে থাকা EIIN কোড বাদ দেওয়া (যেমন: "125057 - DINAJPUR ZILLA SCHOOL")
    name = re.sub(r"^\d{6,8}\s*[-:\s]+", "", name)

    # কমা (,) বা ড্যাশ (-) দিয়ে পেছনের জেলা/থানার অংশ আলাদা করা
    parts = [p.strip() for p in re.split(r"[,|]", name) if p.strip()]
    if len(parts) > 1:
        first_part = parts[0]
        # প্রথম অংশে যদি স্কুলের সাধারণ কি-ওয়ার্ড থাকে, তবে শুধু প্রথম অংশ নেওয়া হবে
        if any(
...
def extract_school_name(soup, raw_text):
    """HTML এবং রেসপন্স টেক্সট থেকে স্কুলের নাম নিখুঁতভাবে বের করা"""
...
def parse_institute_summary(raw_html, eiin_code):
    ...
    # ২. থানা ও জেলার নাম ফিক্সড Regex দিয়ে স্ক্যান
    thana_match = re.search(
        r"(?:Thana|Upazila|Upazilla)\s*[:\-]\s*([A-Za-z0-9\s]+?)(?=\s*(?:District|EIIN|Appeared|Passed|Result|\d|<|$))",
        raw_html,
        re.IGNORECASE,
    )
    if thana_match:
        info["Thana/Upazilla"] = thana_match.group(1).strip()

    dist_match = re.search(
        r"District\s*[:\-]\s*([A-Za-z0-9\s]+?)(?=\s*(?:Thana|Upazila|EIIN|Appeared|Passed|Result|\d|<|$))",
        raw_html,
        re.IGNORECASE,
    )
    if dist_match:
        info["District"] = dist_match.group(1).strip()

    # ৩. টেবিল ও DOM ট্যাগ স্ক্যান (fallback)
    ...
    for i, el in enumerate(all_elements):
        ...
        if i + 1 < len(all_elements):
            ...
            if (
                "thana" in txt or "upazilla" in txt or "upazila" in txt
            ) and info["Thana/Upazilla"] == "-":
                val = re.sub(
                    r"(?i)(thana|upazilla|upazila)\s*[:\-]\s*",
                    "",
                ...
                if (
                    val
                    and val.lower()
                    not in ["thana", "upazilla", "upazila", "thana/upazila"]
                ):
                    info["Thana/Upazilla"] = val
                elif next_val and next_val.lower() not in [
                    "thana",
                    "upazilla",
                    "upazila",
                ]:
            ...
            elif "district" in txt and info["District"] == "-":
                val = re.sub(
                    r"(?i)district\s*[:\-]\s*", "", el.get_text(strip=True)
                ).strip()
                if val and val.lower() != "district":
                    info["District"] = val
                elif next_val and next_val.lower() != "district":
                    info["District"] = next_val

            elif "appeared" in txt and info["No. of Students Appeared"] == "-":
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        # যদি রোল দিয়ে সার্চ হয় (EIIN সার্চ না হয়)
        ...
        if extracted_pairs:
            ...
            # ব্যাকআপ চেক: কেবল EIIN সার্চের ক্ষেত্রেই ইনস্টিটিউট নাম রেজাল্ট ডাটা থেকে সেট হবে
            ...
            if institute_info.get("Percentage of Pass") in [
                "-",
                "0",
                "0.0",
                "",
