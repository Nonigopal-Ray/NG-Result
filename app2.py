...
# ---------------------------------------------------------
MAX_THREADS = 6  # সার্ভার এরোর কমাতে থ্রেড সংখ্যা অপটিমাইজ করা হয়েছে
...
def parse_institute_summary(raw_html, eiin_code):
    ...
    # ১. সরাসরি Regex দিয়ে পাসের হার এক্সট্র্যাক্ট করার সর্বাত্মক চেষ্টা
    pass_match = re.search(
        r"(?:Pass\s*Rate|Percentage\s*of\s*Pass|Pass\s*\(%\)|Pass\s*Percentage)\s*[:\-]?\s*([\d\.]+)",
        raw_html,
        re.IGNORECASE,
    )
    if pass_match:
        info["Percentage of Pass"] = pass_match.group(1)
    ...
    # ২. DOM ট্রাভার্সাল দিয়ে থানা ও জেলা এক্সট্র্যাক্ট করা
    for tr in soup.find_all("tr"):
        cells = tr.find_all(["td", "th"])
        for idx, cell in enumerate(cells):
            cell_txt = cell.get_text(strip=True).lower()
    ...
    # ৩. যদি Regex এ পাসের হার না পাওয়া যায়, তবে DOM Elements লুপ করে চেক করা
    if info["Percentage of Pass"] == "-":
    ...
    try:
        ...
        info["No. of Students Failed"] = str(max(0, app - pas))
    except Exception:
        pass
...
def fetch_student_detail(
    roll, action_url, session_headers, year="2026", initial_status="-", max_retries=6
):
    """
    সার্ভার এরোর প্রতিরোধের জন্য শক্তিশালী অটো-রিট্রাই লুপ।
    রেজাল্ট না পাওয়া পর্যন্ত নির্ধারিত ধাপ পর্যন্ত বার বার চেষ্টা করবে।
    """
    for attempt in range(max_retries):
        ...
        try:
            ...
            if response.status_code == 200:
                # যদি ৫০০ বা সার্ভার ইন্টারনাল এরোর টেক্সট ফেরত আসে, তবে রিট্রাই করবে
                if "500 Internal Server Error" in response.text or "Server Error" in response.text:
                    time.sleep(1 + attempt * 0.5)
                    continue
                ...
                # সঠিক ডাটা প্রাপ্ত হলে রিটার্ন করবে
                return {
                    "Group": group,
                    "Roll": str(roll),
                    "Name": name,
...
def generate_pdf(dataframe, institute_info, year, is_eiin=True):
    ...
    pdf.cell(46, 6, f"Absent: {institute_info.get('No. of Students Absent', '0')}", border=1, align="C", fill=True)
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        if extracted_pairs:
            ...
            # EIIN অনুসন্ধানে যদি বোর্ড ওয়েবসাইট থেকে পাসের হার না পাওয়া যায় অথবা Individual সার্চ হয়, তবে ক্যালকুলেটেড মান বসবে
            if not is_eiin_search or institute_info.get("Percentage of Pass") in ["-", "0", "0.0", "", None]:
                institute_info["Percentage of Pass"] = str(calculated_pass_rate)
...
if (
    ...
    m4.metric("Absent", info.get("No. of Students Absent", "0"))
