...
def extract_school_name(soup, raw_text):
    """HTML এবং রেসপন্স টেক্সট থেকে স্কুলের নাম নিখুঁতভাবে বের করার ফাংশন"""
    # ১. রেগেক্স প্যাটার্ন: EIIN - SCHOOL NAME (যেমন: 125057 - DINAJPUR ZILLA SCHOOL)
    ...
    # ৩. কার্ড হেডার বা হেডিং ট্যাগ স্ক্যান
...
def parse_institute_summary(raw_html, eiin_code):
    ...
    # স্কুলের নাম এক্সট্র্যাক্ট করা
    extracted_name = extract_school_name(soup, raw_html)
    if extracted_name:
        info["Institute"] = extracted_name

    # থানা, জেলা ও সমারী তথ্য পার্সিং
    ...
    for i, el in enumerate(all_elements):
        ...
        if i + 1 < len(all_elements):
            ...
            if "thana" in txt or "upazilla" in txt or "upazila" in txt:
...
def fetch_student_detail(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # স্টুডেন্ট রেজাল্ট পেজ থেকে স্কুলের নাম এক্সট্র্যাক্ট
...
def generate_pdf(dataframe, institute_info, year):
    ...
    # PDF Headline: স্কুলের নাম স্পষ্ট ও বড় আকারে হেডিং-এ থাকবে
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        if extracted_pairs:
            ...
            # ব্যাকআপ চেক: যদি ইনস্টিটিউট নাম সমারীতে না পাওয়া যায়, তবে রেজাল্ট ডাটা থেকে সেট করা হবে
            if institute_info.get("Institute") in [
                "Unknown School",
                "Unknown Institute",
                "",
                None,
...
if (
    ...
    school_name_display = info.get("Institute", "School Result Sheet").strip()
    if not school_name_display or school_name_display == "Unknown School":
        school_name_display = "School Result Sheet"

    # UI তে স্কুলের নাম প্রদর্শন
    if is_eiin_search:
        ...
        st.subheader(f"🏫 {school_name_display}")
    ...
    st.write("---")
    st.subheader(f"📊 Result Sheet ({year}) - {school_name_display}")
    ...
    # ডাইনামিক PDF ফাইল নেম (স্কুলের নাম যুক্ত)
    clean_school_name = sanitize_filename(school_name_display)
    if is_eiin_search:
        pdf_filename = f"SSC_{year}_Result_{clean_school_name}_{eiin_code}.pdf"
    else:
        pdf_filename = f"SSC_{year}_Result_{clean_school_name}.pdf"
