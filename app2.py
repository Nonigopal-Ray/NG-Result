...
# ---------------------------------------------------------
MAX_THREADS = 4  # স্কেলাবিলিটি ও স্পিড বাড়াতে থ্রেড সংখ্যা কিছুটা বাড়ানো হয়েছে
...
def parse_institute_summary(raw_html, eiin_code):
    ...
    # ২. টেবিল থেকে ফিল্টার করা (যদি রেজেক্স মিস হয়)
    for tr in soup.find_all("tr"):
        cells = tr.find_all(["td", "th"])
        for idx, cell in enumerate(cells):
            cell_txt = cell.get_text(strip=True).lower()
    ...
    # ৩. পাসের হার হিসেব করা (Passed / Appeared * 100)
    try:
        if (
            info["No. of Students Appeared"] != "-"
            and info["No. of Students Passed"] != "-"
        ):
            app_val = int(info["No. of Students Appeared"])
            pass_val = int(info["No. of Students Passed"])
            if app_val > 0:
                calculated_rate = round((pass_val / app_val) * 100, 2)
                info["Percentage of Pass"] = str(calculated_rate)
    except Exception:
        pass
...
def fetch_student_detail(
    roll, action_url, session_headers, year="2026", initial_status="-", max_retries=3
):
    """
    অপটিমাইজড সার্ভার রিকোয়েস্ট সিস্টেম:
    অপ্রয়োজনীয় অতিরিক্ত রিট্রাই কমিয়ে ৩ বারে আনা হয়েছে, যাতে সার্ভার এররে ঝুলন্ত অবস্থা না থাকে।
    """
    ...
    for attempt in range(max_retries):
        ...
        try:
            if str(year) == "2026":
                get_res = session.get(action_url, headers=custom_headers, timeout=6)
                ...
                response = session.post(
                    action_url, data=payload, headers=custom_headers, timeout=8
                )
            else:
            ...
            if response.status_code == 200:
                ...
                for i, td in enumerate(all_tds):
                    ...
                    if txt in ["A+", "A", "A-", "B", "C", "D", "F", "ABS", "ABSENT"]:
                        if i - 1 >= 0:
                            ...
                            if len(sub) > 2 and sub.lower() not in [
                                "result",
                                "gpa",
                                "total mark",
                                "group",
                ...
                # ফেলের সংখ্যা সঠিকভাবে নির্ধারণ
                if "ABS" in result_val.upper() or "ABSENT" in result_val.upper():
                    result_val = "ABSENT"
                elif failed_subjects_count > 0:
                    result_val = f"F{failed_subjects_count}"
                elif "FAIL" in result_val.upper() or result_val.strip().upper() == "F":
                    result_val = (
                        f"F{failed_subjects_count}" if failed_subjects_count > 0 else "F1"
                    )
    ...
    return {
        ...
        "Roll": str(roll),
        "Name": "Server Timeout",
        "GPA": initial_status if initial_status != "-" else "Server Error",
        "Total Mark": "-",
        "Subject Grade": "-",
        "School": "",
    }
...
def generate_pdf(dataframe, institute_info, year, is_eiin=True):
    ...
    pdf.cell(
        46,
        6,
        f"Absent: {institute_info.get('No. of Students Absent', '-')}",
        border=1,
        align="C",
        fill=True,
    )
    ...
    if not raw_input:
        ...
        try:
            if selected_year == "2026":
                get_res = session.get(
                    institute_url, headers=headers, timeout=8
                )
                soup_init = BeautifulSoup(get_res.text, "html.parser")
                token_input = soup_init.find("input", {"name": "_token"})
                csrf_token = token_input["value"] if token_input else ""
                ...
                res = session.post(
                    institute_url, data=payload, headers=headers, timeout=10
                )
            else:
                payload = {
                ...
                res = session.post(
                    institute_url, data=payload, headers=headers, timeout=8
                )
        ...
        if extracted_pairs:
            ...
            # ইন্ডিভিজুয়াল রেজাল্ট খোঁজার সময়ে পাসের হার এবং হিসাব
            if not is_eiin_search:
