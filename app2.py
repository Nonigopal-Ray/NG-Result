...
# Custom PDF Class with Tiny Image in Footer
class ColorPDF(FPDF):
    ...
    def footer(self):
        ...
        # Draw tiny developer image if available (8mm x 8mm size)
        if self.img_bytes:
            try:
...
def generate_pdf(dataframe, institute_info, year):
    # Download developer image in memory for PDF placement
    img_stream = None
    try:
    ...
    pdf = ColorPDF(year=year, img_bytes=img_stream, orientation="L", unit="mm", format="A4")
    pdf.alias_nb_pages()
