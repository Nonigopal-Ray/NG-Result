...
def get_board_urls(exam_type, year):
    ...
    else:  # HSC - URL format has no underscore
...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Dinajpur Board Result Check System",
    page_icon="🎓",
    layout="wide"
)
...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    # Sending comprehensive payloads to work seamlessly with both SSC & HSC endpoints
    payload = {
        'roll_no': str(roll),
        'roll': str(roll),
        'exam_roll': str(roll),
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 1. Flexible Parsing Strategy for Name, Group, GPA, Total Mark
            for row in all_rows:
                tds = row.find_all('td')
            ...
            # Fallback Regex Extractions for HSC Edge Cases
            ...
            for row in all_rows:
                ...
                # Format: [CODE] | [SUBJECT] | [GRADE/MARK]
                if len(tds) == 3:
...
# Updated Defaults: SSC as Default Exam, 2026 as Default Year
with col_exam:
