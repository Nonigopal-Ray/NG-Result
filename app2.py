...
def parse_institute_summary(raw_html, eiin_code):
    ...
    # ১. সরাসরি রেজেক্স দিয়ে ওয়েবসাইট থেকে এক্সট্র্যাক্ট করা
    app_match = re.search(
        r"(?:Appeared|Total\s*Appeared|No\.\s*of\s*Students\s*Appeared)\s*[:\-]?\s*(\d+)",
        raw_html,
        re.IGNORECASE,
    )
    if app_match:
        info["No. of Students Appeared"] = app_match.group(1)

    pass_cnt_match = re.search(
        r"(?:Passed|Total\s*Passed|No\.\s*of\s*Students\s*Passed)\s*[:\-]?\s*(\d+)",
        raw_html,
        re.IGNORECASE,
    )
    if pass_cnt_match:
        info["No. of Students Passed"] = pass_cnt_match.group(1)

    fail_cnt_match = re.search(
        r"(?:Failed|Total\s*Failed|No\.\s*of\s*Students\s*Failed)\s*[:\-]?\s*(\d+)",
        raw_html,
        re.IGNORECASE,
    )
    if fail_cnt_match:
        info["No. of Students Failed"] = fail_cnt_match.group(1)

    absent_cnt_match = re.search(
        r"(?:Absent|Total\s*Absent|No\.\s*of\s*Students\s*Absent)\s*[:\-]?\s*(\d+)",
        raw_html,
        re.IGNORECASE,
    )
    if absent_cnt_match:
        info["No. of Students Absent"] = absent_cnt_match.group(1)

    gpa5_match = re.search(
        r"(?:GPA\s*5|GPA-5|GPA5|No\.\s*of\s*GPA5)\s*[:\-]?\s*(\d+)",
        raw_html,
        re.IGNORECASE,
    )
    if gpa5_match:
        info["No of GPA5"] = gpa5_match.group(1)

    pass_rate_match = re.search(
        r"(?:Pass\s*Rate|Percentage\s*of\s*Pass|Pass\s*\(%\)|Pass\s*Percentage|পাসের\s*হার)\s*[:\-]?\s*([\d\.]+)",
        raw_html,
        re.IGNORECASE,
    )
    if pass_rate_match:
        info["Percentage of Pass"] = pass_rate_match.group(1)

    # ২. টেবিল সেল ব্যাকআপ ফিল্টার (যদি রেজেক্স মিস হয়)
    for tr in soup.find_all("tr"):
        cells = tr.find_all(["td", "th"])
        for idx, cell in enumerate(cells):
            cell_txt = cell.get_text(strip=True).lower()
            if idx + 1 < len(cells):
                ...
                if info["No. of Students Appeared"] == "-" and "appeared" in cell_txt:
                    m = re.search(r"\d+", val)
                    if m:
                        info["No. of Students Appeared"] = m.group(0)

                if info["No. of Students Passed"] == "-" and "passed" in cell_txt:
                    m = re.search(r"\d+", val)
                    if m:
                        info["No. of Students Passed"] = m.group(0)

                if info["No. of Students Failed"] == "-" and "failed" in cell_txt:
                    m = re.search(r"\d+", val)
                    if m:
                        info["No. of Students Failed"] = m.group(0)

                if info["No. of Students Absent"] == "-" and "absent" in cell_txt:
                    m = re.search(r"\d+", val)
                    if m:
                        info["No. of Students Absent"] = m.group(0)

                if info["Percentage of Pass"] == "-" and (
                    "pass rate" in cell_txt or "percentage" in cell_txt
                ):
                    m = re.search(r"[\d\.]+", val)
                    if m:
                        info["Percentage of Pass"] = m.group(0)

                if info["No of GPA5"] == "-" and (
                    "gpa 5" in cell_txt or "gpa5" in cell_txt
                ):
                    m = re.search(r"\d+", val)
                    if m:
...
def fetch_student_detail(
    roll, action_url, session_headers, year="2026", initial_status="-", max_retries=15
):
    custom_headers = session_headers.copy()
    custom_headers["Referer"] = action_url

    for attempt in range(max_retries):
        session = requests.Session()
        try:
            if str(year) == "2026":
                get_res = session.get(action_url, headers=custom_headers, timeout=12)
                if get_res.status_code != 200:
                    time.sleep(1 + attempt * 0.4 + random.uniform(0.1, 0.5))
                    continue
                ...
                captcha_answer = solve_captcha(captcha_q.text) if captcha_q else "4"
                ...
                response = session.post(
                    action_url, data=payload, headers=custom_headers, timeout=12
                )
            else:
            ...
            if response.status_code == 200:
                if (
                    "500 Internal Server Error" in response.text
                    or "Server Error" in response.text
                    or "502 Bad Gateway" in response.text
                ):
                    time.sleep(1 + attempt * 0.5 + random.uniform(0.1, 0.5))
                    continue
                ...
                for i, td in enumerate(all_tds):
                    txt = td.text.strip()
                    if txt in ["A+", "A", "A-", "B", "C", "D", "F", "ABS", "ABSENT"]:
                        if i - 1 >= 0:
                ...
                if "ABS" in result_val.upper() or "ABSENT" in result_val.upper():
                    result_val = "ABSENT"
                ...
                if name != "Not Found" or result_val != initial_status:
                    return {
                        "Group": group,
                        "Roll": str(roll),
                        "Name": name,
...
def generate_pdf(dataframe, institute_info, year, is_eiin=True):
    ...
    pdf.cell(46, 6, f"Appeared: {institute_info.get('No. of Students Appeared', '-')}", border=1, align="C", fill=True)
    pdf.cell(46, 6, f"Passed: {institute_info.get('No. of Students Passed', '-')}", border=1, align="C", fill=True)
    pdf.cell(46, 6, f"Failed: {institute_info.get('No. of Students Failed', '-')}", border=1, align="C", fill=True)
    pdf.cell(46, 6, f"Absent: {institute_info.get('No. of Students Absent', '-')}", border=1, align="C", fill=True)
    pdf.cell(46, 6, f"Pass Rate: {institute_info.get('Percentage of Pass', '-')}%", border=1, align="C", fill=True)
    pdf.cell(47, 6, f"GPA 5: {institute_info.get('No of GPA5', '-')}", border=1, align="C", fill=True)
    pdf.ln(8)
    ...
    for _, row in dataframe.iterrows():
        ...
        is_fail = "F" in gpa or "ABS" in gpa.upper()
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        if extracted_pairs:
            ...
            # কেবল ইন্ডিভিজুয়াল রেজাল্টের অনুসন্ধানের ক্ষেত্রেই ম্যানুয়ালি হিসাব করা হবে
            if not is_eiin_search:
                ...
                passed_count = sum(
                    ...
                    and str(g) not in ["-", "Server Error", "Server Timeout", "No Result Found"]
                )
                failed_count = sum(1 for g in df["GPA"] if "F" in str(g).upper())
                gpa5_count = sum(1 for g in df["GPA"] if str(g) in ["5.00", "5"])
                
                total_stu_cnt = len(df)
                calc_rate = (
                    round((passed_count / total_stu_cnt) * 100, 2)
                    if total_stu_cnt > 0
                    else 0
                )

                institute_info["No. of Students Appeared"] = str(total_stu_cnt)
                institute_info["No. of Students Passed"] = str(passed_count)
                institute_info["No. of Students Failed"] = str(failed_count)
                institute_info["No. of Students Absent"] = str(absent_count)
                institute_info["Percentage of Pass"] = str(calc_rate)
...
if (
    ...
    m4.metric("Absent", info.get("No. of Students Absent", "-"))
    ...
    for _, row in df.iterrows():
        ...
        for item in str(row["Subject Grade"]).split(","):
            item_str = item.strip()
            if ":F" in item_str or ":ABS" in item_str:
                sg_formatted.append(
