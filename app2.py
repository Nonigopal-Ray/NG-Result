...
def create_robust_session():
    """Creates a requests session with auto-retry logic to handle connection errors."""
...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Parse Table Rows & Cells flexibly for both SSC & HSC layout variations
            ...
            for row in all_rows:
                ...
                # Check Name
                if re.search(r'Name\s*(?:of\s*Student)?\s*[:\-]', row_text, re.IGNORECASE):
                    tds = row.find_all('td')
                    for j, td in enumerate(tds):
                        if "name" in td.text.lower() and j + 1 < len(tds):
                ...
                # Check GPA / Result Status
                if "gpa" in row_text.lower() or "result" in row_text.lower():
                    tds = row.find_all('td')
                    for j, td in enumerate(tds):
            ...
            # Fallback for Name / Group / GPA if row parsing didn't catch it
            if name == "Not Found" or group == "Not Found" or total_mark_val == "-":
                for i, td in enumerate(all_tds):
                    txt = td.text.strip().lower()
                    if name == "Not Found" and "name" in txt and i + 1 < len(all_tds):
                        possible_name = all_tds[i+1].text.strip().lstrip(':').strip()
                        if len(possible_name) > 2 and not possible_name.isdigit():
                            name = possible_name
                    if group == "Not Found" and "group" in txt and i + 1 < len(all_tds):
                        possible_group = all_tds[i+1].text.strip().lstrip(':').strip()
                        if possible_group: 
                            group = possible_group
                    if total_mark_val == "-" and "total mark" in txt and i + 1 < len(all_tds):
                        possible_tm = all_tds[i+1].text.strip().lstrip(':').strip()
                        if possible_tm: 
                            total_mark_val = possible_tm

            # Parse Subject Grades (Supports HSC structure: Subject name and Grade next to it or under it)
            subject_grades = []
            ...
            for row in all_rows:
                ...
                if len(tds) >= 2:
                    ...
                    # Pattern 2: [Code/Sl] | [Subject Name] | [Grade]
                    elif len(tds) >= 3:
                        t3 = tds[2].text.strip()
                        if t3 in ['A+', 'A', 'A-', 'B', 'C', 'D', 'F'] and len(t2) > 2:
                            subject_grades.append(f"{t2}:{t3}")
                            if t3 == 'F':
                                if not any(opt in t2.lower() for opt in OPTIONAL_SUBJECT_KEYWORDS):
                                    compulsory_failed_count += 1
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        try:
            ...
            res = session.post(institute_url, data=payload, headers=headers, timeout=15)
            raw_text = res.text
