...
def get_board_urls(exam_type, year):
    """Dynamically generate exact Board URLs for SSC and HSC."""
    ...
    else:  # HSC - Notice that HSC URLs do NOT have an underscore between 'result' and 'year'
...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Dinajpur Board HSC & SSC Result Check System",
    page_icon="🎓",
    layout="wide"
)
...
# Updated Header Title as Requested
st.markdown("<h1>🎓 Dinajpur Board HSC & SSC Result Check System</h1>", unsafe_allow_html=True)
...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    # Sending multi-parameter payloads to handle both SSC and HSC board server keys
    payload = {
        'roll_no': str(roll),
        'roll': str(roll),
        'exam_roll': str(roll),
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Check if result really doesn't exist
            ...
            # 1. Parse Primary Student Information
            for row in all_rows:
                tds = row.find_all('td')
                row_text = row.get_text(separator=" ", strip=True)
                
                # Extract Name
                if "Name of Student" in row_text or "Name" in row_text:
                    for j, td in enumerate(tds):
                        txt = td.text.strip().lower()
                        if "name" in txt and j + 1 < len(tds):
                            val = tds[j+1].text.strip().lstrip(':').strip()
                            if val and not val.isdigit() and "father" not in val.lower():
                                name = val
                                break
            ...
            # Fallbacks via Regex (Matching exact format from photo)
            if name == "Not Found":
                match_name = re.search(r'Name\s*of\s*Student\s*:\s*([^|\n\r]+)', text_content, re.IGNORECASE)
                if match_name:
            ...
            if total_mark_val == "-":
                match_tm = re.search(r'TOTAL\s*MARK\s*:?\s*(\d+)', text_content, re.IGNORECASE)
                if match_tm:
            ...
            for row in all_rows:
                ...
                # HSC Table Format: [CODE] | [SUBJECT] | [MARK/GRADE]
                if len(tds) == 3:
                ...
                # SSC Table Format: [SUBJECT] | [GRADE]
                elif len(tds) == 2:
                    t1 = tds[0].text.strip()
                    t2 = tds[1].text.strip()
...
if 'df_results' in st.session_state and not st.session_state['df_results'].empty:
    ...
    exam_type = st.session_state.get('selected_exam', 'HSC')
