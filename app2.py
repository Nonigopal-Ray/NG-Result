...
def clean_thana_upazila(raw_val):
    """থানা/উপজেলার নাম ক্লিন করার বিশেষ ফিক্সড ফাংশন"""
    if not raw_val:
        return "-"
    # Pre-clean prefixes
    cleaned = re.sub(
    ...
    # Break on structural separators
    cleaned = re.split(r"[,\|\n\r\t<>]", cleaned)[0].strip()
    # Keep only english alphabet letters and spaces
...
def parse_institute_summary(raw_html, eiin_code):
    ...
    # ২. থানা/উপজেলা নিখুঁতভাবে ফিল্টার করা (DOM Traversal)
    for tr in soup.find_all("tr"):
        cells = tr.find_all(["td", "th"])
        for idx, cell in enumerate(cells):
            cell_txt = cell.get_text(strip=True).lower()
            if any(k in cell_txt for k in ["thana", "upazila", "upazilla"]):
                if idx + 1 < len(cells):
                    val = cells[idx + 1].get_text(strip=True)
    ...
    # ফলব্যাক Regex (যদি টেবিলে না পাওয়া যায়)
    if info["Thana/Upazilla"] == "-":
        match = re.search(
    ...
    dist_match = re.search(
        r"District\s*[:\-]\s*([A-Za-z\s]{3,20})", raw_html, re.IGNORECASE
    )
    if dist_match:
        info["District"] = dist_match.group(1).strip().upper()

    # ৪. অপরাপর পরিসংখ্যান তথ্য পার্সিং
